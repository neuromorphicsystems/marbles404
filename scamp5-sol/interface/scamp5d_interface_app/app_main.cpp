...
// other global variables used by this example
...
// 'packet_switch' setup code is put into a sepereate function to keep the initilization code cleaner
void setup_packet_switch(){
    ...
    packet_switch->case_analog([&](int width,int height,uint8_t*bitmap_buffer){
        auto sn = packet_switch->get_packet_sn();
        auto lc = packet_switch->get_loop_counter();
        // auto display = packet_switch->get_display_handle();
...
int main(int argc,char*argv[]){
    ...
    printf("Scamp Interface Example\n");
...
#if 1
    // connect to a real box via USB
    box = new scamp5d_usb();
    r = box->open("", -1);
#else
    // connect to a proxy (e.g. Host App/Simulation) via TCP
    box = new scamp5d_tcp();
    ...
    /// Start the TCP Proxy
    // A proxy is not essential, but it allows the program get connected 
    // by another interface application to pass down the data.
