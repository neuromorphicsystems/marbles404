...
PyObject *decode_packet(uint8_t*packet_buffer,size_t packet_size){
    ...
    packet_switch.ret = NULL;
    auto r = packet_switch.decode_packet(packet_buffer,packet_size,packet_counter++);

    if(r!=python_module_packet_switch::done || packet_switch.ret==NULL){
        ...
        packet_switch.ret = Py_BuildValue("{ss,si,sy#}",
            "type","unknown",
            "size",payload_size,
            "payload",payload,size_t(payload_size)
        );
    }
