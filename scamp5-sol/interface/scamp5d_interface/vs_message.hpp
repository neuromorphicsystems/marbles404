...
struct vs_device_error_bits_t{
	uint32_t booting_failed:1;
	uint32_t shutdown_failed:1;
	uint32_t watchdog_timeout:1;
	uint32_t coroutine_timeout:1;
	uint32_t bank0_malloc_failed:1;
	uint32_t bank1_malloc_failed:1;
	uint32_t shared_malloc_failed:1;
	uint32_t icw_stack_overflow:1;
	uint32_t calibration_failed:1;
};
#pragma pack(pop)
...
#define VS_MSG_GUI_UPDATE_SLIDER_MIN   	140
#define VS_MSG_GUI_UPDATE_SLIDER_MAX   	141
#define VS_MSG_CLEAR_TEXT  		142
