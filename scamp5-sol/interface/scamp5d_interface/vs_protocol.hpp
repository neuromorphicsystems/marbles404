...
#define VS_DATA_DISPLAY_TEXT    	24
#define VS_DATA_DISPLAY_IMAGE    	25
#define VS_DATA_DISPLAY_DRAW     	26
...
struct vs_data_display_text{
	int16_t kpos_c;
	int16_t kpos_r;
	uint8_t color_rgba[4];
	size_t len;
	char str[1];// dummy size
};

struct vs_gui_imageformat_t{
	uint16_t pixel_bytes:4;
	uint16_t is_bottom_to_top:1;
	uint16_t is_right_to_left:1;
	uint16_t is_column_major:1;
	uint16_t is_channel_array:1;
};
